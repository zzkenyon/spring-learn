...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/** Indicates 是否注册了InstantiationAwareBeanPostProcessors  bean实例化前后的后置处理器 */
	private boolean hasInstantiationAwareBeanPostProcessors;

	/** Indicates 是否注册了DestructionAwareBeanPostProcessors bean销毁前后的后置处理器 */
	...
	protected Object getObjectForBeanInstance(
		...
		//且beanInstance也不是FactoryBean对象
		if (BeanFactoryUtils.isFactoryDereference(name) && !(beanInstance instanceof FactoryBean)) {
			//name != null  && name.startWith("&") && !(beanInstance instanceof FactoryBean)
			throw new BeanIsNotAFactoryException(transformedBeanName(name), beanInstance.getClass());
		}

		//如果Bean实例不是FactoryBean，或者指定名称是容器的解引用，
		//调用者向获取对容器的引用，则直接返回当前的Bean实例
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		//name以"&"开头 获取的是FactoryBean对象
		//代码执行到这说明name不是以"&"开头，且beanInstance是FactoryBean对象
		//处理指定名称不是容器的解引用，或者根据名称获取的Bean实例对象是一个工厂Bean
		//使用工厂Bean创建一个Bean的实例对象
		Object object = null;
		if (mbd == null) {
			//从Bean工厂缓存中获取给定名称的Bean实例对象  获取到的是Factory
